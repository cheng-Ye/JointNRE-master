...
class NN(object):
	...
	def __init__(self, is_training, word_embeddings, simple_position = False):
		...
		self.num_classes = int(FLAGS.num_classes)
		...
		with tf.name_scope("embedding-layers"):
			# word embeddings
			temp_word_embedding = tf.get_variable(initializer=word_embeddings[int(FLAGS.ent_total):,:],name = 'temp_word_embedding',dtype=tf.float32)
	...
	def katt(self, x, is_training = True, dropout = True):
		with tf.name_scope("knowledge-based-attention"):
			...
			for i in range(int(FLAGS.batch_size)):
				sen_matrix = x[self.scope[i]:self.scope[i+1]]
				attention_score = tf.nn.softmax(tf.reshape(attention_logit[self.scope[i]:self.scope[i+1]], [1, -1]))
				final_repre = tf.reshape(tf.matmul(attention_score, sen_matrix),[int(self.output_size)])
				tower_repre.append(final_repre)
			if dropout:
				stack_repre = tf.layers.dropout(tf.stack(tower_repre), rate = self.keep_prob, training = is_training)
			else:
	...
	def katt_test(self, x, is_training = False):
		...
		kg_att = tf.concat([each_att for i in range(int(self.num_classes))], 2)
	...
	def __init__(self, is_training, word_embeddings, simple_position = False):
		...
		if not is_training:
			with tf.name_scope("test"):
				...
				test_tower_output = []
				for i in range(int(FLAGS.test_batch_size)):
				...
				test_stack_output = tf.reshape(tf.stack(test_tower_output),[int(FLAGS.test_batch_size), self.num_classes])
